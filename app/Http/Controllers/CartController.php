...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        if($already_cart) {
            ...
            $already_cart->amount = $discountedPrice + $already_cart->amount;
    ...
    public function singleAddToCart(Request $request){
        ...
        if($already_cart) {
            ...
            $already_cart->amount = $discountedPrice * $already_cart->quantity;
            ...
            $cart->amount = $discountedPrice * $request->quant[1];
            if ($cart->product->stock < $cart->quantity || $cart->product->stock <= 0) return back()->with('error','Stock not sufficient!.');
            // return $cart;
            $cart->save();
        }
        ...
        if($request->quant){
            ...
            foreach ($request->quant as $k=>$quant) {
                ...
                if($quant > 0 && $cart) {
                    ...
                    $cart->price = $discountedPrice;
                    $cart->amount = $discountedPrice * $cart->quantity;
                    // return $cart->price;
                    $cart->save();
    ...
    public function checkout(Request $request)
    {
        ...
        $liveOngkirConfigured = $this->rajaOngkir->isConfigured();
        $cartWeight = Helper::cartWeight();

        return view('frontend.pages.checkout', compact('user', 'lastOrder', 'firstName', 'lastName', 'liveOngkirConfigured', 'cartWeight'));
    }
}
