...
class FrontendController extends Controller
{
    ...
    public function productGrids(Request $request){
        ...
        if ($request->filled('sortBy')) {
            ...
            switch ($sortBy) {
                case 'title':
                    $products = $products->where('status', 'active')->orderBy('title', $direction);
                    break;
                case 'price':
                ...
                case 'created':
                    ...
                    break;
                default:
                    $products = $products->where('status', 'active')->orderBy('id', 'DESC');
            }
        } else {
            $products = $products->where('status', 'active')->orderBy('id', 'DESC');
        }
    ...
    public function productFilter(Request $request){
            ...
            if(!empty($data['sortBy'])){
                $sortVal = $data['sortBy'];
                if (in_array($sortVal, ['title', 'price', 'created'])) {
                    $sortByURL .= '&sortBy=-' . $sortVal;
                } else {
                    $sortByURL .= '&sortBy=' . $sortVal;
                }
            }
