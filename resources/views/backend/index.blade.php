...
@section('title','ECSECOND || DASHBOARD')
    ...
    <div class="row">
      ...
      <div class="col-xl-4 col-lg-5">
        <div class="card shadow mb-4">
          ...
          <div class="card-body">
            @if($bestSellingProductRows->isEmpty())
              <div class="d-flex align-items-center justify-content-center text-muted" style="height:320px;">
                No sales data yet
              </div>
            @else
              @php
                $maxSold = max(1, (int) $bestSellingProductRows->max('total_sold'));
              @endphp
              <div class="pt-2 pb-1" style="min-height:320px;">
                @foreach($bestSellingProductRows as $product)
                  @php
                    $sold = (int) $product->total_sold;
                    $percentage = max(8, round(($sold / $maxSold) * 100));
                  @endphp
                  <div class="mb-4">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                      <span class="small font-weight-bold text-gray-800" title="{{ $product->product_name }}">
                        {{ \Illuminate\Support\Str::limit($product->product_name, 34) }}
                      </span>
                      <span class="small text-gray-600">{{ $sold }} sold</span>
                    </div>
                    <div class="progress" style="height:18px;">
                      <div class="progress-bar bg-primary" role="progressbar" style="width: {{ $percentage }}%;" aria-valuenow="{{ $sold }}" aria-valuemin="0" aria-valuemax="{{ $maxSold }}"></div>
                    </div>
                  </div>
                @endforeach
              </div>
